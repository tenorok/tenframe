...
class ten_file {
    ...
    public static function upload_image($files, $options = array()) {
        ...
        foreach($files_array as $num => $file) {
            ...
            ten_file::make_dir($options['path']);                                    // Создание пути, если его не существует
    ...
    private static function merge_file($mod, $val, $options) {
        ...
        if($mod == 'fls') {                                                          // Если переданы конкретные файлы для объединения
            ...
            foreach($input_path as $path) {                                          // Цикл по входящим директориям
                ...
                $options['input_path'] = txt::rgum(ROOT . $path, '/');               // Абсолютный путь входящей корневой директории и добавление слеша в конец пути, если его там нет
    ...
    public static function include_files($files, $options = null) {
        ...
        if(!empty($options['output_file'])) {                                        // Если указан файл для сохранения результата
            ...
            $output_file = ten_file::autogen(                                        // Сохранение файла
                $options['output_file'],
                (ten\core::$settings['compressHTML']) ? ten\tpl::compressHTML($included) : $included,
