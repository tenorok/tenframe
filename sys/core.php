	...
	Парсинг blitz-шаблонов:
		echo core::block(array(										// Функция всегда принимает в качестве параметра массив
			...
			'context' => array(										// Массив контекстов begin-end
				...
				'context4' => array(...)							// Количество контекстов не ограничено
			)
		));
...
class core {
	...
	private static function context($tpl, $ctx, $val) {
		...
		if(gettype($val) == 'array') {											// Если контекст нужно проитерировать
			...
			if(!isset($val['array']) && !empty(core::$ctx_last_array_element))	// Если у текущего контекста не задан массив к перебору и существует последний элемент предыдущего контекста
				$val['array'] = array(core::$ctx_last_array_element);			// то текущий контекст нужно отпарсить в соответствии с текущим элементов контекста родителя

			foreach($val['array'] as $element) {								// Цикл по массиву значений к присваиванию
				
				if(
					isset($val['if'])  && !$element[$val['if']] ||				// Если задано положительное условие и оно не выполняется
					isset($val['!if']) &&  $element[$val['!if']]				// или задано отрицательное условие и оно не выполняется
				)
				...
				foreach($val as $ctx2 => $arr)									// Цикл по элементам контекста
					if(
						$ctx2 != 'array' &&										// Если ключ не является служебной переменной array
						$ctx2 != 'parse' ||										// и не является служебной переменной parse
